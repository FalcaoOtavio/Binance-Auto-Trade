...
def coletar_dados():
    ...
    candles = binance.fetch_ohlcv('DOGE/USDT', timeframe='1h', limit=100)
    df = pd.DataFrame(candles, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
    df = calcular_macd(df)
    df_final = df[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'MACD', 'Signal']]
    csv_file = './Binance-Trade-Auto-DOGE/files/binance_data_with_macd.csv'
...
def main():
    ...
    model = load_model('./Binance-Trade-Auto-DOGE/files/ia.h5')
    ...
    model_retreinado.save('./Binance-Trade-Auto-DOGE/files/ia.h5')
